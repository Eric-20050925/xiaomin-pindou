...
配色风格提供八种选择。默认的协调美化会整理色阶、控制饱和度并优先复用协调色号；保真还原以原始感知色差为准；鲜明强化会增强明暗和饱和度，适合印章、像素画与海报；卡通风格会将明暗和色相归并为更清晰的扁平色块，并减少零碎近似色。切换风格或松开颜色滑块后会自动重新生成图纸。

创意风格还包括柔彩马卡龙、复古暖调、清冷通透和黑白剪影。柔彩马卡龙提亮暗部并降低色彩刺激；复古暖调压缩反差、加入温暖的黄红倾向；清冷通透提高亮度并偏向蓝青；黑白剪影移除色相并归并为明确的灰度层级。

工作台的“风格预览”会渐进生成同一图片的八种配色结果，同时显示各自使用的颜色数量。当前风格会立即显示，其余风格逐张补齐；点击任意预览会直接应用已生成的图纸，不再重复计算。
